...
class FederationUninstaller:
    ...
    def remove_wrapper_scripts(self):
        ...
        if wrapper_dir.exists():
            try:
                ...
                print(f"  Could not remove wrappers: {e}")
    ...
    def update_configuration(self):
        ...
        if not self.config_path.exists():
            print("  No configuration to update")
            return True
    ...
    def restore_backup_if_exists(self):
        ...
        if self.backup_path.exists():
            ...
            print("  No backup found (using updated configuration)")
            return False
    ...
    def handle_database(self):
        ...
        if db_path.exists():
            ...
            if response == 'y':
                ...
                print("  Database preserved (can be reused if reinstalling)")
        else:
            print("  No unified database found")
    ...
    def display_summary(self):
        ...
        if self.failed_removals:
            print(f"\nFailed to remove: {len(self.failed_removals)} MCPs")
    ...
    def uninstall(self):
        ...
        try:
            ...
            if not self.create_safety_backup():
                response = input("\nContinue without safety backup? (y/N): ").lower().strip()
                if response != 'y':
                    print("\nUninstallation cancelled")
                    return False
