...
function Install-Dependencies {
    ...
    if (-not $pythonCmd) {
        ...
        if (-not (Test-AdminRights)) {
            Write-Host "    WARNING: Administrator rights required for Python installation" -ForegroundColor Red
    ...
    if (-not $nodeCmd) {
        ...
        if (-not (Test-AdminRights)) {
            Write-Host "    WARNING: Administrator rights required for Node.js installation" -ForegroundColor Red
            Write-Host "    Please run this script as Administrator" -ForegroundColor Yellow
            exit 1
        }
        ...
        Write-Host "    OK: Found: node $(node --version 2>$null)" -ForegroundColor Green
    }
    ...
    if (-not $gitCmd) {
        ...
        if (-not (Test-AdminRights)) {
            Write-Host "    WARNING: Administrator rights required for Git installation" -ForegroundColor Red
            Write-Host "    Please run this script as Administrator" -ForegroundColor Yellow
            exit 1
        }
        ...
        Write-Host "    OK: Found: git $(git --version 2>$null)" -ForegroundColor Green
    }
    ...
    if ($needsRestart) {
        ...
        Write-Host "  WARNING: New software installed. You may need to restart your terminal." -ForegroundColor Yellow
...
function Update-Repository {
    ...
    if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
        Write-Host "    WARNING: Git not available, skipping repository clone" -ForegroundColor Yellow
        return $false
    }
...
if ($WhatIf) {
    ...
    Write-Host "SUCCESS: Claude Desktop is now supercharged with 15 MCPs!" -ForegroundColor Cyan
    Write-Host ""
}